...
# Typical use: keep plugins, mods and configs versioned in git.
#
# Startup variables (defined in egg-minecraft-multi.json):
...
#                           (a restart loads them; the server is never killed
...
#     workspace never holds a .git, so root-installed/panel-booted ownership
#     mismatches and stale git locks cannot break updates.
#   * File-level manifest: only files the repo previously shipped are removed
#     on update, so server-written state (plugin userdata, world data) inside
#     synced directories is never touched.
...
# Server-written state inside synced directories must never be removed even
# when a directory itself is repo-managed (e.g. plugins/ from git).
_pf_sync_is_state_file() {
    case "$1" in
        */userdata/*|*/userdata|*/playerdata/*|*/playerdata|*/stats/*|*/stats|*/advancements/*|*/advancements|*.lock|session.lock)
            return 0 ;;
    esac
    return 1
}
...
# can never clobber or wipe live credentials and the project keeps working.
# Set GIT_PRESERVE_ENV=0 to let the repository's .env files win instead.
...
# overwrite - e.g. GIT_EXCLUDE="custom/* secrets". Everything else is synced
# normally.
...
sync_git_repo() {
    ...
    if [ "${has_manifest}" = "1" ] && [ -n "${last_commit}" ]; then
        ...
        # but never server-written state files.
        while IFS= read -r f; do
            [ -n "${f}" ] || continue
            _pf_sync_is_protected "${f}" && continue
            _pf_sync_is_state_file "${f}" && continue
...
start_git_update_watcher() {
    ...
    GIT_AUTO_UPDATE_PID=$!
    ok "Git Auto-Update watcher active (polling every ${GIT_POLL_SECONDS:-300}s; new commits are synced - restart the server to load them)."
}
...
start_stop_watcher() {
    ...
    if [ "${STOP_WATCHER_PID}" -eq 0 ]; then
        ...
        if ( exec 9<&0 ) 2>/dev/null; then
            ...
            # `2>/dev/null` here silently re-pointed the launcher's (and the
            # server's) stderr to /dev/null for the whole run. The subshell
            # probe above already guarantees this dup succeeds.
            exec 3<&0
            panel_stop_watcher &
            STOP_WATCHER_PID=$!
